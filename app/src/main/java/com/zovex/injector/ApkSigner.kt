...
class ApkSigner(private val context: Context) {
    ...
    fun sign(unsigned: File, out: File) {
        ...
        ZipOutputStream(out.outputStream().buffered()).use { zos ->
            zos.setLevel(6)
            fun put(n: String, d: ByteArray) { zos.putNextEntry(ZipEntry(n)); zos.write(d); zos.closeEntry() }
    ...
    private fun buildManifest(apk: File): ByteArray {
        ...
        ZipFile(apk).use { zip ->
            zip.entries().asSequence().filter { !it.name.startsWith("META-INF/") }.sortedBy { it.name }.forEach { e ->
    ...
    private fun buildPkcs7(cert: X509Certificate, sig: ByteArray): ByteArray {
        ...
        val si=seq(i1(1),seq(cert.issuerX500Principal.encoded,sn),seq(sha,nil),seq(rsa,nil),oct(sig))
    ...
    private fun generateCert(kp: KeyPair): X509Certificate {
        // ניסיון 1: sun.security.x509
        return try { generateViaSun(kp) }
        catch (e: Exception) {
            // ניסיון 2: BouncyCastle
            try { generateViaBC(kp) }
            catch (e2: Exception) {
                // ניסיון 3: DER ידני
                generateViaRaw(kp)
            }
    ...
    private fun generateViaSun(kp: KeyPair): X509Certificate {
        ...
        val v = Class.forName("sun.security.x509.CertificateValidity")
        ...
        val n = Class.forName("sun.security.x509.X500Name")
            .getConstructor(String::class.java).newInstance("CN=ZovexInjector,O=Zovex,C=IL")
        val a = Class.forName("sun.security.x509.AlgorithmId")
        ...
        set.invoke(ci, "serialNumber", cls("sun.security.x509.CertificateSerialNumber")
            .getConstructor(BigInteger::class.java).newInstance(BigInteger(64, SecureRandom())))
        set.invoke(ci, "subject", cls("sun.security.x509.CertificateSubjectName")
            .getConstructor(cls("sun.security.x509.X500Name")).newInstance(n))
        set.invoke(ci, "issuer", cls("sun.security.x509.CertificateIssuerName")
            .getConstructor(cls("sun.security.x509.X500Name")).newInstance(n))
        set.invoke(ci, "key", cls("sun.security.x509.CertificateX509Key")
            .getConstructor(PublicKey::class.java).newInstance(kp.public))
        set.invoke(ci, "algorithmID", cls("sun.security.x509.CertificateAlgorithmId")
            .getConstructor(cls("sun.security.x509.AlgorithmId")).newInstance(a))
        set.invoke(ci, "version", cls("sun.security.x509.CertificateVersion")
    ...
    private fun generateViaBC(kp: KeyPair): X509Certificate {
        ...
        val g = gen.javaClass
        val x = Class.forName("$bc.asn1.x500.X500Name")
            .getConstructor(String::class.java).newInstance("CN=ZovexInjector,O=Zovex,C=IL")
        val now = java.util.Date()
        val exp = java.util.Date(now.time + 3650L*86400_000L)
        g.getMethod("setSerialNumber", BigInteger::class.java).invoke(gen, BigInteger(64, SecureRandom()))
        for (m in listOf("setIssuerDN","setSubjectDN"))
            g.getMethod(m, Class.forName("$bc.asn1.x500.X500Name")).invoke(gen, x)
        ...
        g.getMethod("setSignatureAlgorithm", String::class.java).invoke(gen, "SHA256WithRSAEncryption")
        return g.getMethod("generate", PrivateKey::class.java).invoke(gen, kp.private) as X509Certificate
    }

    private fun generateViaRaw(kp: KeyPair): X509Certificate {
        fun len(n: Int) = when { n<0x80->byteArrayOf(n.toByte()); n<0x100->byteArrayOf(0x81.toByte(),n.toByte()); else->byteArrayOf(0x82.toByte(),(n ushr 8).toByte(),(n and 0xFF).toByte()) }
        fun w(t: Int, d: ByteArray) = byteArrayOf(t.toByte()) + len(d.size) + d
        fun seq(vararg p: ByteArray) = w(0x30, p.reduce{a,b->a+b})
        fun oid(vararg v: Int) = w(0x06, v.map{it.toByte()}.toByteArray())
        ...
        fun time(ms: Long): ByteArray {
            val s = java.text.SimpleDateFormat("yyMMddHHmmss'Z'", java.util.Locale.US).also {
                it.timeZone = java.util.TimeZone.getTimeZone("UTC")
            }.format(java.util.Date(ms))
            return w(0x17, s.toByteArray())
        }
        val name = seq(seq(seq(oid(0x55,0x04,0x03), w(0x0C,"ZovexInjector".toByteArray()))))
        val tbs = seq(
            ctx(0,i1(2)),
            BigInteger(64,SecureRandom()).toByteArray().let{byteArrayOf(0x02)+len(it.size)+it},
            seq(sha256rsa,nil), name,
            seq(time(now-1000), time(now+3650L*86400_000L)),
            name, kp.public.encoded
        )
        val sig = Signature.getInstance("SHA256withRSA").also{it.initSign(kp.private);it.update(tbs)}.sign()
        val certDer = seq(tbs, seq(sha256rsa,nil), w(0x03, byteArrayOf(0x00)+sig))
